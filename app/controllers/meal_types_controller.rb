class MealTypesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @meal_type.update(meal_type_params)
        format.html { redirect_to @meal_type, notice: "Meal type was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to meal_types_path, notice: "Meal type was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_meal_type
      @meal_type = MealType.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def meal_type_params
      params.expect(meal_type: [ :name ])
    end
end
